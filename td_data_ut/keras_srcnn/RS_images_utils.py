...
def block_png_imgs(img_path, stander_size):
    ...
    u8_mat = np.array(im,dtype=np.uint8)
    block_mat_list, block_mat_init_location = block_mat_stander_size(u8_mat, stander_size, stander_size)
    return block_mat_list, block_mat_init_location

def block_info_mat(u8_mat,stander_block_w = 1024,stander_block_h = 1024 ):
    # print u8_mat.shape
    ...
    return block_w, block_h, block_w_s, block_h_s
...
def block_rgb_mat(u8_mat,stander_block_w = 1024,stander_block_h = 1024 ):
    ...
    for w_idx in range(block_w_s):
        for h_idx in range(block_h_s):
            ...
            # dest_local = {}
            # dest_local['W'] = (w_idx + 1) * block_w
            # dest_locall['W'] = (w_idx + 1) * block_w
...
def block_mat_stander_size(u8_mat,stander_block_w = 1024,stander_block_h = 1024 ):
    print (u8_mat.shape)
    block_w_s_neg = np.int(np.float(u8_mat.shape[0]) / np.float(stander_block_w))
    block_h_s_neg = np.int(np.float(u8_mat.shape[1]) / np.float(stander_block_h))
    
    block_w = np.int(stander_block_w)
    block_h = np.int(stander_block_h)
    block_w_s = np.int(block_w_s_neg)
    block_h_s = np.int(block_h_s_neg)
...
def turn_gray_mat_uint16_uint8(uint16_iomat):
    """
    :param uint16_iomat: uint16 np.mat
    :return: uint8 mat
    """
    float_iomat = uint16_iomat.astype(np.float)
    float_gray_mat = hist_2_98(float_iomat[:,:,np.newaxis])
    float_gray_mat = float_gray_mat.reshape([float_gray_mat.shape[0],float_gray_mat.shape[1]])
    print('floa_gray_mat shape :', float_gray_mat.shape)
    # float_mat = np.concatenate([float_r_mat,float_g_mat,float_b_mat],axis=2)
    u8_mat = float_gray_mat.astype(np.uint8)
    return u8_mat
...
def save_tiff_png(tiff_path,png_path):
    ...
    outmat = turn_mat_uint16_uint8(iomat,c_r = 2, c_g = 1, c_b = 0)
    img_save = Image.fromarray(outmat, mode='RGB')
    img_save.save(png_path)
    return

def save_gray_tiff_png(tiff_path,png_path):
    """
    :param tiff_path: multi channel RS images, uint16,  tiff , path
    :param png_path: RGB channel RS images, uint8  png , path
    :return: save uint16 tiff file as uint8 png file
    """
    iomat = tifffile.imread(tiff_path)
    outmat = turn_gray_mat_uint16_uint8(iomat)
    img_save = Image.fromarray(outmat, mode='L')
    img_save.save(png_path)
    return
...
if __name__=='__main__':
    ...
    block_info_tiff(im_path)
    # block_mat_init_location = block_tiff_img(im_path,stander_size=1024)
    # for ini_loca in block_mat_init_location:
    #     print ini_loca
    # for zip_iter in enumerate(zip(block_mat_list, block_mat_init_location)):
    #     print zip_iter[0].shape
    #     print zip_iter[1]
