...
if __name__=='__main__':
    ...
    for i,mat_path_iter in enumerate(mat_path_list):
        ...
        if mat_path_iter[-4:] == '.mat':
            ...
            rpg_path = os.path.join(unzip_floder_path,mat_path_iter[:-4]+'.rpb')
            jpg_path = os.path.join(unzip_floder_path,mat_path_iter[:-4]+'.jpg')
            ...
            if os.path.isfile(xml_path):
                ...
                png_path = os.path.join(target_path_iter, mat_path_iter[:-4]+'.tiff')
                if not os.path.exists(target_path_iter):
                    os.mkdir(target_path_iter)
                
                print('target path', target_path_iter)
                turn_mat_2_tif(mat_path,png_path)
                shutil.copy(xml_path,target_path_iter)
                shutil.copy(rpg_path,target_path_iter)
                shutil.copy(jpg_path,target_path_iter)
