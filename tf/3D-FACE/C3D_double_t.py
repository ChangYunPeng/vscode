...
from C3D_MODEL import C3D_ENCODER,C3D_DECODER,C2D_ENCODER,C2D_DECODER,Attention_Model, Attention_Model_Xt, Attention_Convert_Model, C3D_Test_ENCODER
...
class C3D_Running:
    ...
    def build_model_adn_loss_opt(self):
            ...
            with tf.device('/gpu:%d' % self.selected_gpu_num):
                with tf.name_scope('GPU%d' % self.selected_gpu_num) as scope:
                    ...
                    with tf.name_scope('encoder'):
                        ...
                        gray_encoder1 = self.gray_encoder(self.gray_train_in1_ph,self.phase)
                        gray_encoder2 = self.gray_encoder(self.gray_train_in2_ph,self.phase)
                    ...
                    with tf.name_scope('net_variable'):
                        self.gray_trainable_variable = self.gray_encoder.trainable_variable 
        ...
        tf.summary.image('gray_encoder_f1', self.gray_train_in1_ph[:,0,:,:,:])
        tf.summary.image('gray_encoder_f2', self.gray_train_in1_ph[:,1,:,:,:])
        tf.summary.image('gray_encoder_f3', self.gray_train_in2_ph[:,0,:,:,:])
        tf.summary.image('gray_encoder_f4', self.gray_train_in2_ph[:,1,:,:,:])
        tf.summary.image('gray_encoder_difference', tf.clip_by_value(tf.square(gray_frame_0 - gray_frame_1),0.0,1.0))

        tf.summary.image('gray_inputer_difference1', tf.square((self.gray_train_in1_ph[:,0,:,:,:]-self.gray_train_in1_ph[:,1,:,:,:])))
        tf.summary.image('gray_inputer_difference2', tf.square((self.gray_train_in2_ph[:,0,:,:,:]-self.gray_train_in2_ph[:,1,:,:,:])))
...
run_model.train_c3d(400000, restore_tags=False, trainable_whole=True, trainable_mid = True)
