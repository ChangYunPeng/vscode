...
class C3D_Running:
    def __init__(self):
        ...
        self.root_path = '/media/room304/TB/TensorFlow_Saver/ANORMLY/COMPARED_Patch_End/'
        ...
        with tf.device('/cpu:0'):
            ...
            with tf.device('/gpu:%d' % self.selected_gpu_num):
                with tf.name_scope('GPU%d' % self.selected_gpu_num) as scope:
                    ...
                    # gray
                    split_axis_h = tf.split(self.gray_train_in_ph, num_or_size_splits=h_bands, axis=2)
                    split_axis_hw_list = []
                    ...
                    for split_axis_h_iter in split_axis_hw_list:
                        ...
                        for split_axis_hw_iter in split_axis_h_iter:
                            _,tmp_out = self.gray_c3d_model(split_axis_hw_iter, self.phase)
                    ...
                    self.gray_train_out_ph = tf.concat(merge_axis_h_list,axis=2)

                    # optical flow
                    split_axis_h = tf.split(self.optical_train_in_ph, num_or_size_splits=h_bands, axis=2)
                    split_axis_hw_list = []
                    for split_axis_h_iter in split_axis_h:
                        split_axis_hw_list.append(tf.split(split_axis_h_iter, num_or_size_splits=w_bands, axis=3))
                    ...
                    for split_axis_h_iter in split_axis_hw_list:
                        tmp_hw_list = []
                        for split_axis_hw_iter in split_axis_h_iter:
                            _,tmp_out = self.optical_c3d_model(split_axis_hw_iter, self.phase)
                    ...
                    self.optical_train_out_ph = tf.concat(merge_axis_h_list,axis=2)
