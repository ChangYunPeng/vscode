...
class C3D_Running:
    def __init__(self):
        ...
        self.root_path = '/home/room304/TB/TB/TensorFlow_Saver/ANORMLY/Split_C2D_Light_Extraloss/'
    ...
    def build_model_adn_loss_opt(self):
            ...
            with tf.device('/gpu:%d' % self.selected_gpu_num):
                with tf.name_scope('GPU%d' % self.selected_gpu_num) as scope:
                    ...
                    with tf.name_scope('train_loss'):
                        ...
                        self.total_loss =  -self.att_loss + self.extra_loss_ratio *(att1_extra_loss + att2_extra_loss)
        ...
        tf.summary.image('difference', self.out_image_difference)
    ...
    def train_c3d(self,max_iteration = 100000 , restore_tags = True, trainable_whole=True, trainable_mid = True, ):
        ...
        summaries_dir = self.summaries_dir + 'slice_temporal_to_compare_model_light_extraloss%d.CPTK' % time.time()
