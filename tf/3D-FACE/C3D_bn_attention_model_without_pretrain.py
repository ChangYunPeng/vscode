...
class C3D_Running:
    ...
    def build_model_adn_loss_opt(self):
        ...
        out_image_heatmap_normalize = (heat_map - tf.reduce_min(heat_map))/( tf.reduce_max(heat_map) - tf.reduce_min(heat_map) )
        out_image_heatmap_clip = tf.clip_by_value(heat_map, 0, 1.0)
        self.out_image_heat_map = heat_map
        self.heat_map_max =tf.reduce_min(heat_map)
        
        out_image_gray_output = tf.clip_by_value(tf.reshape(self.gray_train_out_ph, shape = [-1 , self.img_size_h , self.img_size_w , 1 ]),0,1.0)

        out_image_gray_diff_output = tf.clip_by_value(tf.reshape(self.gray_loss_sequences_pixel_mean, shape = [-1 , self.img_size_h , self.img_size_w , 1 ]),0,1.0)

        out_image_optical_diff_output = tf.clip_by_value( tf.reshape(self.optical_loss_sequences_pixel_mean,shape=[-1 , self.img_size_h , self.img_size_w , 1]), 0, 1.0)

        self.out_image = tf.concat([out_image_gray_output, out_image_gray_diff_output,out_image_optical_diff_output ], axis=2)
        self.out_heat_map = tf.concat([out_image_heatmap_normalize,out_image_heatmap_clip], axis=2)
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            while 1:
                ...
                if not (batches == []):
                    ...
                    out_image1, out_image2 ,out_heap_map, out_hp_min= sess.run([self.out_image, self.out_heat_map,self.out_image_heat_map,self.heat_map_max],
                        feed_dict={ self.train_in_ph : batches, self.phase: False})
                    print(out_hp_min)
                    image_batches_list1.append(out_image1)
                    image_batches_list2.append(out_image2)
                    img_hp.append(out_heap_map)
                else:
                    break
            
            image_batches_list1 = np.concatenate(image_batches_list1,axis=0)
            image_batches_list2 = np.concatenate(image_batches_list2,axis=0)
            img_hp = np.concatenate(img_hp,axis=0)
            img_hp = img_hp/img_hp.max()
            image_batches_list2 = np.concatenate([image_batches_list2,img_hp], axis=2)
            # print(image_batches_list1.shape)
            # print(image_batches_list1.shape)
            save_batch_images(image_batches_list1,self.gray_img_save_path,'gray_datasets_%d_video_%d.jpg'%(seletced_dataset_idx,selected_video_idx))
            save_batch_images(image_batches_list2,self.optical_img_save_path,'heatmap_datasets_%d_video_%d.jpg'%(seletced_dataset_idx,selected_video_idx))
