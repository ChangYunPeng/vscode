...
from C3D_MODEL import C3D_ENCODER,C3D_DECODER,C2D_ENCODER,C2D_DECODER,Attention_Model, Attention_Model_Xt, Attention_Convert_Model, Attention_sigmoid_Model
...
class C3D_Running:
    def __init__(self):
        ...
        self.mid_model = Attention_sigmoid_Model( input_channel = 3,model_scope_name = 'net',s_size=4, attention_uints = 1024, attention_hops = 512)
        ...
        self.root_path = '/home/room304/TB/TB/TensorFlow_Saver/ANORMLY/Attention_xt_convert/'
    ...
    def build_model_adn_loss_opt(self):
        ...
        with tf.device('/cpu:0'):
            ...
            with tf.control_dependencies(update_ops):
                ...
                self.optical_train_in_ph =  tf.placeholder(dtype=tf.float32,shape=[None, self.video_imgs_num, self.img_size_h,self.img_size_w, 2], name='batches_optical')
            ...
            with tf.device('/gpu:%d' % self.selected_gpu_num):
                with tf.name_scope('GPU%d' % self.selected_gpu_num) as scope:
                    ...
                    with tf.name_scope('mid_stage_squeeze'):
                        ...
                        mid_stage_out_gray = mid_stage_out_p #tf.expand_dims(gray_encoder,axis=1)
                    ...
                    with tf.name_scope('train_loss'):
                        ...
                        self.total_loss = self.optical_loss* self.optical_flow_loss_ratio + self.gray_loss* self.gray_loss_ratio  + extraloss * self.extra_loss_ratio 
    ...
    def fetch_net_loss(self, sess, batch_data):
        train_loss = {}
        gray_loss,tmp_sum = sess.run([self.gray_loss,  self.summary_merged], feed_dict={self.train_in_ph : batch_data,self.phase : False})
    ...
    def train_c3d(self,max_iteration = 100000 , restore_tags = True, trainable_whole=True, trainable_mid = True, ):
        my_multi_train_datasets = multi_train_datasets(batch_size = self.batch_size, video_num = self.video_imgs_num, frame_interval = 0, is_frame = True, is_Optical = False,crop_size=4, img_size=self.img_size_h)
        gpu_options = tf.GPUOptions(allow_growth=True)
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            for idx in range(max_iteration):  
                ...
                if (idx+1)%1000 == 0 and (idx+1) >=2000:
                # if (idx+1)%50 == 0:
...
run_model.train_c3d(200000, restore_tags=False, trainable_whole=True, trainable_mid = True)
