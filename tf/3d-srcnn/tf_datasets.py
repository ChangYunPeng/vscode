...
def _parse_function(filename):
  ...
  image_cropped = tf.image.resize_image_with_crop_or_pad(image_decoded,32,32)
  image_cropped = tf.image.convert_image_dtype(image_cropped,tf.float32)

  image_resized = tf.image.resize_images(image_cropped, [16, 16])
  image_resized = tf.image.resize_images(image_resized, [32, 32])
